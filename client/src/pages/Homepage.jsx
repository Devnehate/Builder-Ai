    ...
    <div className="h-screen overflow-y-scroll text-white font-sans bg-[url('/bg-img.png')] bg-cover bg-center bg-no-repeat">
      <nav className='sticky top-0 z-10 flex items-center justify-between px-6 py-4'>
        <div className='flex items-center gap-2'>
          ...
          <span className='text-left font-semibold tracking-tight'>BuilderAI</span>
        </div>
      ...
      <div className='flex-1 flex flex-col items-center justify-center px-6 pb-20 mt-8 xl:mt-28'>
        <div className='w-full max-w-2xl flex flex-col items-center'>
          ...
          <div className='w-full mt-6'>
            <Promptinput onSubmit={handleGenerate} loading={generatingProjects} placeholder='Create a portfolio website...' variant='glass' autoFocus />
          </div>

          <div className='masked-marquee w-full mt-4 max-w-2xl overflow-hidden py-1'>
            <div className='animate-marquee gap-3'>
              {homeTags.map((tag, index) => (
                <button key={index} onClick={() => handleGenerate(tag)} disabled={generatingProjects} className='px-4 py-1.5 border rounded-full text-sm text-white bg-white/10 border-white/25 hover:bg-white/20 transition cursor-pointer shrink-0'>
                  {tag}
                  </button>
                ))}
            </div>
          ...
          {!loadingProjects && projects?.length > 0 && (
            <div className='mt-12 w-full'>
              ...
              <div className='space-y-2 max-h-[80vh] overflow-y-auto pr-1'>
                {projects.map((p) => (
                  <div key={p._id} className='bg-white/5 border border-white/10 rounded-lg px-4 py-3 flex items-center justify-between group hover:border-white/20 hover:bg-white/10 cursor-pointer backdrop-blur-md transition-all' onClick={()=> navigate(`/builder/${p._id}`)}>
